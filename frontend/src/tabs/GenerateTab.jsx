...
import { generateScript, simulateScript } from "../api/client";
...
export default function GenerateTab() {
  ...
  const generate = async () => {
    ...
    setSim(null);
    try {
      const data = await generateScript({ prompt, os_profile: osProfile, language });
      setResult(data);
    } catch {
  ...
  const simulate = async () => {
    ...
    setSimLoad(true);
    try {
      const data = await simulateScript({ prompt });
      setSim(data);
    } catch {
